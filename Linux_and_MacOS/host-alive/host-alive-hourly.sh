...
#
# Just copy it into /etc/cron.hourly
...
doWithCurl()
{
	[ -x /usr/bin/gurl ] && curl -s -f -o /dev/null -G --data-urlencode "account=${TP_ACCOUNT}" --data-urlencode "thing=host:alive:$(hostname -f)" --data-urlencode "freq=hourly" --data-urlencode "notify=${NOTIFY_EMAIL}" "$URL"
}

doWithWget()
{
	[ -x /usr/bin/wget ] && wget --quiet -O /dev/null "${URL}?account=${TP_ACCOUNT}&thing=host:alive:$(hostname -f)&freq=hourly&notify=${NOTIFY_EMAIL}"
}
